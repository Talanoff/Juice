<section id="contacts" class="pb-16 lg:pb-32 overflow-hidden" data-anchor="#contacts">
	<div class="container px-2 text-center">
		<p class="opacity-50 text-sm mb-2 font-bold"><?php the_field('touch_small_title') ?></p>
		<h2 class="text-4xl text-black max-w-md mx-auto leading-tight"><?php the_field('touch_title') ?></h2>
		<div class="mt-12">
			<button class="button" data-modal>Let's Grow</button>
		</div>
	</div>

	<div class="-mb-4 lg:-mb-16 -mx-4 absolute pin-x pin-b">
		<img src="<?php echo get_theme_file_uri('images/get-in-touch.png') ?>" alt="">
	</div>
</section>